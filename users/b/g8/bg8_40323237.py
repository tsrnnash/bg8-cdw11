...
bg8_40323237 = Blueprint('bg8_40323237', __name__, url_prefix='/bg8_40323237', template_folder='templates')
...
'''
    return outstring
@bg8_40323237.route('/BADC')
def BADC():
    return head_str + chain_str + BADC(0, 0) + tail_str 

# 傳繪 A 函式內容
def BADC(x, y, scale=1, color="gray"):
    outstring = '''
# 利用 chain class 建立案例, 對應到 mychain 變數
mychain = chain(scale='''+str(scale)+''', fillcolor="'''+str(color)+'''")
...
'''
    return outstring
@bg8_40323237.route('/twocircle/<x>/<y>')
@bg8_40323237.route('/twocircle', defaults={'x':0, 'y':0})
def twocircle(x,y):
...
'''
    ...
    return outstring
@bg8_40323237.route('/twocircle2/<x>/<y>')
@bg8_40323237.route('/twocircle2', defaults={'x':0, 'y':0})
def twocircle2(x,y):
